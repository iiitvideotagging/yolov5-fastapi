...
def imageInput(device, src):
    ...
    elif src == 'From BDD100K Dataset.':
        # Image selector slider
        imgpath = glob.glob('data/images/*')
        imgsel = st.slider('Select random images from sample BDD100K images.', min_value=1, max_value=len(imgpath), step=1)
...
def videoInput(device, src):
    if src == 'Upload your own data.':
        ...
        if uploaded_video != None:
            ...
            if device == 'cuda':
                detect(weights=cfg_model_path, source=imgpath, device=0)
            else:
        ...
        # Image selector slider
        imgpath = glob.glob('data/videos/*')
        imgsel = st.slider('Select random videos from BDD100K dataset', min_value=1, max_value=len(imgpath),step=1)
        uploaded_video = imgpath[imgsel - 1]
        submit = st.button("Predict!")
        outputpath = os.path.join('data/video_output', os.path.basename(imgpath))
...
def main():
    ...
    iou_option = st.sidebar.slider('Select iou threshold', min_value=0.5, max_value=1.0, step=0.05)
    ...
    st.header(':oncoming_automobile: IIIT Object Detection')
