...
def detect_run_updated(
        ...
        track=False,  # run tracking
        show_track=False,  # show tracked path
        show_fps=False,  # show fps
        thickness=2,  # bounding box and font size thickness
        seed=1,  # random seed to control bbox colors
        nobbox=False,  # don`t show bounding box
        nolabel=False,  # don`t show label
        unique_track_color=False,  # show each track in unique color
        stframe=None,
        kpi5_text="",
        kpi6_text="",
):
    ...
    # Richard , Initialize sort object: In v5, parse_opt is a function, moved initialization in Run. 
    np.random.seed(seed)
    ...
    if not nosave:  # Richard , added if condition
    ...
    bbox_colors = [[random.randint(0, 255) for _ in range(3)] for _ in names]
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # Stream results
            # im0 = annotator.result() # Richard - Not available in v7
            if view_img:
